defmodule VBT.Telemetry.Oban do
  ...
  def install_handler do
    events = Enum.map([:stop, :exception], &[:oban, :job, &1])
    :telemetry.attach_many(__MODULE__, events, &handle_event/4, nil)
  end

  defp handle_event([:oban, :job, :stop], _measure, meta, nil) do
    Logger.debug(~s/processed job id=#{meta.id} in queue #{inspect(meta.queue)}/)
  end

  defp handle_event([:oban, :job, :exception], _measure, meta, nil) do
    Logger.error("""
    failed processing job id=#{meta.id} in queue #{inspect(meta.queue)}:

      #{Exception.format(normalize_kind(meta.kind), meta.error, meta.stacktrace)}
    """)
  end
